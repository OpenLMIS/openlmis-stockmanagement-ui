...
* [OLMIS-8361](https://openlmis.atlassian.net/browse/OLMIS-8361): Leaving a line item row on the Issue, Receive, Adjustments and Unpack Kit screens now marks its empty or invalid fields, with the same checks as Submit. Before, an untouched row showed nothing until Submit. A newly added row that the user has not clicked or typed in yet is not checked. In packs mode an empty quantity is reported as required on leave and on Submit, instead of being turned into 0 packs.

...
* [OLMIS-8362](https://openlmis.atlassian.net/browse/OLMIS-8362): Issue, Receive and Adjustment can be opened offline without having been visited online first. The post-login warm-up now goes through the same full stock card summary repository those screens use, so the orderable fulfills, orderables and lots they resolve are cached too, and it waits for every supported program instead of firing the queries and returning.
